    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex justify-between items-start mb-8">
          <span className={`text-[9px] font-mono font-black uppercase tracking-[0.3em] px-4 py-1.5 rounded-xl border-2 ${difficultyStyles[problem.difficulty]}`}>
            {problem.difficulty}
          </span>
          <div className="flex flex-wrap gap-2 justify-end">
            {problem.tags.slice(0, 2).map((tag, index) => (
              <span 
                key={index} 
                className="text-[8px] font-mono font-black uppercase tracking-widest bg-white/5 text-slate-500 px-3 py-1.5 rounded-lg border-2 border-white/5"
              >
                #{tag}
              </span>
            ))}
            {problem.tags.length > 2 && (
              <span className="text-[8px] font-mono font-black text-primary/40 px-2 py-1.5 uppercase tracking-widest">
        ...
        <p className="text-[13px] text-slate-500 leading-relaxed line-clamp-3 mb-10 font-bold uppercase tracking-tight">
          {problem.description}
        </p>
        ...
        >
          ...
          <ArrowRight className="w-5 h-5 text-slate-600 group-hover/btn:text-black group-hover/btn:translate-x-2 transition-all" />
        </button>
      </div>
